...
workers, retains events coded for All foreign nationalities, and compares aggregate destination-year models with a
...
aggregate design cannot absorb. The preferred bilateral
...
permits (beta = 0.005446, p = 0.004, N = 6,301; wild-cluster bootstrap
p = 0.027). The corresponding PPML estimate for the absolute
high-skilled permit count is also positive and statistically significant
(beta = 0.036814, p = 0.012), while the total employment-permit count is
not. The work-visa-only share-model narrowing gives beta = 0.006642 with
p = 0.0025. This pattern is consistent with dual-track migration systems
shaping who continues to enter when general labor-migration rules become
more or less restrictive, but it does not identify the causal effect of
any dedicated high-skill route.
...
High-skilled inflows contribute to innovation, productivity, and fiscal
sustainability. They are particularly important as European labor
markets face persistent shortages, while existing evidence finds only
small effects on native wages (Hunt and Gauthier-Loiselle 2010; Peri
...
Selection theory supports the high-skilled-share outcome. Research on
policy effectiveness motivates the focus on restrictiveness, while the
channel-substitution and gravity literatures support the bilateral
...
or low-skilled workers. Fourth, it estimates a denominator-weighted
bilateral origin-destination-year model with destination and origin-year
fixed effects. Fifth, it uses wild-cluster bootstrap inference to
address the limited number of destination clusters. These choices
sharpen the analysis, but the estimates remain conditional associations
...
preferred bilateral specification, it is absorbed by origin-year fixed
effects.
...
model has no origin or citizenship-group index. Equation (3) retains
individual origins, indexed by o. Alpha denotes destination fixed
effects, lambda year fixed effects, and beta is the coefficient on
...
nationalities. CumPolicyHS and CumPolicyLS control for
reforms that directly target the high-skilled numerator or the
lower-skilled part of the denominator. PermitScale controls for the
permit denominator's size, not total employment in the destination
economy.
...
weighted estimate remains significant at the five-percent
level: the cluster-robust t-statistic is 3.17 and the bootstrap
two-sided p-value is 0.027.
...
individual citizenship rows using a fixed EU27-2020 classification: the
Non-EU group contains citizenship codes outside those 27 members, while
the EU27 group contains those members and excludes the reporting country.
This keeps group membership constant over time, but it also means that UK
citizenship is classified outside the EU27 group throughout the window
and Croatia inside it throughout; this convention is treated as a
measurement limitation rather than a time-varying legal-exposure rule.
...
labor-migration target group; target origin All foreign nationalities (permit outcome years: 2008--2020) &
...
The retained events comprise 250 skilled/high-skilled reforms, 61
low-skilled reforms, and 180 all-migrant-worker reforms. The
all-migrant-worker score is the focal measure; the two skill-targeted
scores enter as controls.

\Needspace{7\baselineskip}\captionof{table}{Policy event counts under three target-group definitions, target origin All foreign nationalities, 2007--2020.}\label{tab:event-counts}
...
\bottomrule\noalign{}
\endlastfoot
Skilled/high-skilled targeted only & 250 & 154 & 230 & 1 & 19 \\
Low-skilled targeted only & 61 & 38 & 39 & 0 & 22 \\
All-migrant-worker-only & 180 & 92 & 113 & 3 & 64 \\
\end{longtable}
}
...
Work visa/permit is the largest category (92 events), covering permit
...
related adjustments, while entry-visa/stay rules (21 events, including
...
\bottomrule\noalign{}
\endlastfoot
Work visa/permit & 92 & 24 & Core channel for legal labor entry and
work authorization \\
...
legal stay or work \\
Entry visa/stay permit (including source label ``Entry visa and stay'') & 21 & 1 &
...
The empirical base is sizeable. After the main Eurostat filters are
applied, the residence-permit data contain 305,448 long-format rows
before aggregation and 1,274 constructed outcome rows across citizenship
...
Table 5 reports descriptive statistics for the complete-control
aggregate and bilateral HSRate samples. Alternative outcomes are
reported with their corresponding results.
...
Lagged all-migrant-worker policy score & -1.208 & 4.489 & -16.000 &
8.000 & 308 \\
Lagged annual all-migrant-worker policy flow & -0.344 & 1.977 & -10.000
& 8.000 & 308 \\
...
Lagged all-migrant-worker policy score & -1.170 & 5.024 & -16.000 &
8.000 & 6,301 \\
Lagged high-skill-targeted policy score & -9.899 & 8.854 & -34.000 &
3.000 & 6,301 \\
Lagged low-skill-targeted policy score & -0.123 & 1.695 & -6.000 &
4.000 & 6,301 \\
...
\captionof{figure}{Descriptive relationship between cumulative net policy-change score and HSRate within 2008--2020. Graph 1A shows each destination's first-to-last available endpoint change within the window; seven series do not contain both exact boundary years. Graph 1B reports cumulative policy paths for the nine largest destinations by total Non-EU employment permits in Eurostat over 2008--2020. The United Kingdom is marked separately because of Brexit. Graph 1A covers 31 descriptive series, whereas the main controlled regressions retain 30 destination clusters after the lagged-control complete-case filter.}\label{graph:descriptive}
...
all-migrant-worker cumulative coefficient is \(\beta = 0.001273\)
with cluster-robust SE = 0.002050 (t = 0.62, p = 0.540, N = 308, G =
30). The annual policy-flow alternative is also not significant
(\(\beta = 0.000459\), SE = 0.002472, p = 0.854), and the aggregate
log high-skilled-count outcome is not significant either
(\(\beta = 0.020727\), SE = 0.026339, p = 0.438). The aggregate
...
Lagged all-migrant-worker policy score
  & -0.000605 & -0.000280 & 0.000944 & 0.001273 & 0.020727 & 0.000459 \\
\textit{Cluster-robust SE}
  & (0.001956) & (0.002003) & (0.001995) & (0.002050) & (0.026339) & (0.002472) \\
Exact p-value
  & 0.759 & 0.890 & 0.640 & 0.540 & 0.438 & 0.854 \\
\addlinespace[1pt]
...
M1 include 2008--2020 because they do not require lagged permit scale.
...
Lagged cumulative all-migrant-worker score
  & 0.005446*** & 0.003933** & 0.004351** & \NA \\
\textit{Cluster-robust SE}
  & (0.001716) & (0.001755) & (0.001624) & \NA \\
Exact p-value
  & 0.004 & 0.033 & 0.012 & \NA \\
...
The preferred weighted coefficient is \(\beta =\) 0.005446 with
cluster-robust SE = 0.001716 (t = 3.17, p = 0.004, N = 6,301
origin-destination-year cells across G = 30 clustered destinations). The
wild-cluster bootstrap p-value is 0.027, so the result remains
...
associated with a 0.54-percentage-point increase in the high-skilled
...
0.001721, p = 0.425), while the low-skilled control is negative but
imprecise (\(\beta =\) -0.005586, p = 0.148). Scaling the coefficient to policy
...
1.63-percentage-point higher high-skilled share the following year,
while a Major restrictive reform contributes plus 4 units and maps to a
2.18-percentage-point increase. Against the regression-sample mean
high-skilled share of 20.9 percent in the bilateral panel, a Major
reform therefore moves the share by roughly 10.4 percent of its mean. By
contrast, the full-control aggregate coefficient is smaller and
imprecise (\(\beta =\) 0.001273, p = 0.540), which would imply only
about a 0.51-percentage-point change for a Major reform and is not
...
remains positive and statistically significant (\(\beta =\) 0.003933, SE
= 0.001755, p = 0.033), and the denominator-weighted model with
origin-destination corridor fixed effects is also positive and
significant (\(\beta =\) 0.004351, SE = 0.001624, p = 0.012). Within the
retained EMP $\geq 30$ sample, the positive association therefore does
not depend only on weighting or on stable corridor differences. Section
8 shows that the weighted result also survives removing the cutoff,
although the unweighted no-cutoff comparison is imprecise. By contrast, the bilateral
annual policy-flow specification with the same skill-target controls is
not significant (\(\beta =\) -0.000107, p = 0.957). The preferred result
...
The results point to a composition association accompanied by evidence on
the high-skilled count, but not on total employment-permit volume. The
high-skilled share is positively associated with the cumulative net
policy-change measure in the preferred denominator-weighted
bilateral specification ($\beta$ = 0.005446, p = 0.004, N = 6,301; bootstrap p
= 0.027), consistent with the joint prediction stated in Section 3. The
...
high-skilled permit count is positive and significant in the PPML
comparison. The evidence therefore supports both a higher relative skill
share and a higher high-skilled count within the retained corridor sample,
without evidence of a change in total employment-permit volume.
...
low-skilled policy controls is not significant (\(\beta =\) 0.001273, p
= 0.540, N = 308), and the aggregate annual-flow version is also not
significant (\(\beta =\) 0.000459, p = 0.854, N = 308). The
...
paths (\(\beta =\) 0.005446, p = 0.004, N = 6,301). The substantive
...
modeled-ILO control, the no-cutoff and all-first-permits checks, the
work-visa-only measure, and applicable EU directive-window controls. The
origin-income results are heterogeneous, the core-tool narrowing is
imprecise, and the aggregate placebo coefficients are not significant.
...
\addlinespace[2pt]
Bilateral: no minimum-cell cutoff & Retains every positive employment-permit denominator & 0.004994*** & 0.004 & Significant; not created by cutoff \\
Bilateral: all-first-permits denominator & Replaces the employment-permit denominator with all first residence permits & 0.000856*** & 0.008 & Significant; survives broader denominator \\
Bilateral: ILOSTAT, no UK & Replaces Eurostat unemployment with ILOSTAT & 0.004950*** & 0.001 & Significant; survives source change \\
Bilateral: ILOSTAT, with UK & Adds the United Kingdom using ILOSTAT unemployment & 0.000579 & 0.838 & Not significant \\
Bilateral: core labor-entry measure & Restricts the policy index to core labor-entry and administrative tools & 0.002948 & 0.127 & Positive; not significant \\
Bilateral: work-visa tool only & Restricts the policy index to work-visa and work-permit events & 0.006642*** & 0.002 & Significant; larger estimate \\
...
Bilateral: EU directive windows & Adds four destination-year directive-window controls where legally applicable & 0.005606*** & 0.003 & Preferred coefficient remains stable \\
\addlinespace[3pt]
\multicolumn{5}{@{}l}{\textbf{Panel B. Aggregate EU27 placebo diagnostics}} \\
\addlinespace[2pt]
Non-EU group, cumulative score & Aggregate treated-group benchmark & 0.001273 & 0.540 & Not significant (N=308; G=30) \\
...
Non-EU group, annual policy flow & Tests newly adopted policy changes against the following year's outcome & 0.000459 & 0.854 & Not significant (N=308; G=30) \\
...
The first sensitivity row removes the minimum-cell cutoff while retaining
the preferred denominator weights. The coefficient remains positive and
significant (\(\beta =\) 0.004994, p = 0.004, N = 23,623), so the weighted
...
0.002377, p = 0.351). This contrast shows that the preferred evidence is
...
reasons. Under the preferred policy specification with skill-target
controls, the preferred-style weighted bilateral coefficient remains
positive and statistically significant
(\(\beta =\) 0.000856, p = 0.008, N = 5,392). The corridor-FE variant is
also significant (\(\beta =\) 0.000897, p = 0.047), while the version
that keeps the original employment-permit scale control remains
significant (\(\beta =\) 0.000941, p = 0.004, N = 6,301). This supports
...
smaller than the preferred estimate and is not statistically significant
(\(\beta =\) 0.002948, p = 0.127). The check therefore supports the
direction of the preferred result, but not its precision after the policy
measure is narrowed.
...
counterpart is also small and imprecise (M3: 0.001273, p = 0.540). Because
...
Eurostat no-UK weighted estimate is \(\beta =\) 0.005446 with SE =
0.001716 (p = 0.004, N = 6,301, G = 30). The ILOSTAT no-UK weighted
estimate is \(\beta =\) 0.004950 with SE = 0.001408 (p = 0.001, N =
...
through ILOSTAT, however, the coefficient falls to \(\beta =\) 0.000579
with SE = 0.002807 (p = 0.838, N = 7,331, G = 31). This supports the
...
significant for high-income origins (\(\beta =\) 0.015211, p = 0.004),
upper-middle-income origins (\(\beta =\) 0.003535, p = 0.042), and
lower-middle-income origins (\(\beta =\) 0.005729, p = 0.005). The
low-income-origin estimate is negative and statistically significant
(\(\beta = -0.003325\), p = 0.037), but this stratum is smaller (N =
...
weight. The work-visa-only coefficient is \(\beta =\) 0.006642 with
cluster-robust SE = 0.002006 (t = 3.31, cluster-robust p = 0.0025). The
coefficient is moderately larger than the all-tools preferred estimate
of 0.005446. This pattern is consistent with the
...
combined applicable-window indicator gives \(\beta =\) 0.005637 with SE
= 0.001766 (p = 0.003). A more demanding variant adds the four
directive-specific dummies separately, which gives \(\beta =\) 0.005606
with SE = 0.001722 (p = 0.003). The Restrict coefficient remains very
close to the preferred estimate of 0.005446 in both variants.
...
directive-specific coefficients from specification (c), and Panel B places each
...
coefficient remains positive and significant (\(\beta =\) 0.005606, p =
0.003), indicating that the main association is not accounted for by
...
Work visa/permit, the largest tool category, is share-positive (0.66
percentage points; p = 0.0025) and broadly consistent with the
...
the unweighted bilateral comparison within the retained sample, corridor
fixed effects, removal of the minimum-cell cutoff under the preferred
weights, the broader all-first-permits denominator, the no-UK ILOSTAT
control source, the work-visa-only narrowing, and EU-directive-window
controls. The aggregate and annual-flow bilateral models do not produce it.

The supporting PPML model also returns a positive, statistically
significant association with the high-skilled permit count, while the
total employment-permit count remains statistically unchanged. The
pattern therefore combines a compositional shift with evidence of higher
high-skilled volume in the retained corridors. It does not show that
overall employment-permit inflows rise, and it remains an association
rather than a causal count effect.
...
equal-cell causal effect. The current denominator may itself respond to
policy, so the weighting scheme can also reweight observations using a
post-policy quantity; the unweighted comparison checks how dependent
the finding is on this choice. The main EMP $\geq 30$ rule also selects
cells using the current denominator. The weighted no-cutoff result remains
significant, but the no-cutoff unweighted comparison does not, so the
evidence is more robust to the cutoff than to the weighting estimand. The evidence is not equally strong
...
citizens in the same way as to third-country nationals, the absence of a
statistically significant EU27 coefficient is directionally reassuring.
The placebo is nevertheless based on only 38 observations and an
imperfect fixed citizenship composition, so it cannot establish that all
destination-wide confounding has been removed.
...
First, the preferred estimate suggests that net restrictive change in
general labor-migration policy is not compositionally neutral in this
sample. The estimated shift is consistent with
high-skilled inflows changing more favorably than other permit inflows.
The PPML result provides evidence of a higher high-skilled count, but it
does not identify a causal count effect or directly observe a separate
lower-skilled count. General all-migrant-worker reforms, such
as tighter work-permit conditions, more demanding employer obligations,
tighter quotas, and narrower access to rights attached to
...
The work-visa-only narrowing produces a larger estimate.
...
associated with roughly a 0.54-percentage-point increase in the
...
the absolute high-skilled permit count, while the total employment-permit
count remains statistically unchanged. Future work should
...
\midrule
Lagged all-migrant-worker policy score
  & -0.000605 & -0.000280 & 0.000944 & 0.001273 & 0.020727 \\
\textit{Cluster-robust SE}
  & (0.001956) & (0.002003) & (0.001995) & (0.002050) & (0.026339) \\
t-statistic
  & -0.31 & -0.14 & 0.47 & 0.62 & 0.79 \\
Exact p-value
  & 0.759 & 0.890 & 0.640 & 0.540 & 0.438 \\
\addlinespace[2pt]
Outcome
...
specification with origin-year fixed effects (beta = 0.005446, p =
0.004). The unweighted controlled bilateral estimate is retained as a
comparison and remains positive and significant under the
fixed-effect-aware correction (beta = 0.003933, p = 0.033).
...
Aggregate cumulative (M3)
  & 0.001273 & 0.002050 & 0.540 & Yes & No & 308 & 30 \\
Aggregate annual policy flow
  & 0.000459 & 0.002472 & 0.854 & Yes & No & 308 & 30 \\
Aggregate log high-skilled count (M4)
  & 0.020727 & 0.026339 & 0.438 & Yes & No & 308 & 30 \\
\addlinespace[2pt]
\textbf{Preferred weighted bilateral}
  & \textbf{0.005446***} & \textbf{0.001716} & \textbf{0.004} & Yes & Yes & 6,301 & 30 \\
\quad High-skill policy control
  & 0.001721 & 0.002126 & 0.425 & -- & -- & 6,301 & 30 \\
\quad Low-skill policy control
  & -0.005586 & 0.003757 & 0.148 & -- & -- & 6,301 & 30 \\
Unweighted bilateral comparison
  & 0.003933** & 0.001755 & 0.033 & Yes & No & 6,301 & 30 \\
Weighted bilateral with corridor FE
  & 0.004351** & 0.001624 & 0.012 & Yes & Yes & 6,301 & 30 \\
Bilateral annual policy flow
  & -0.000107 & 0.001947 & 0.957 & Yes & No & 6,301 & 30 \\
\bottomrule
\end{tabular}
\end{center}
