...
Second, the signed, magnitude-weighted policy measure closes the
measurement gap left by binary policy coding by preserving the direction
and size of each reform. Third, the separation of all-migrant-worker reforms from
...
and the coefficient of interest is beta on CumPolicyAll, the lagged
cumulative net policy-change score for all-migrant-worker reforms.
CumPolicyHS and CumPolicyLS are included as policy controls because they
...
opt-outs, or Croatia's 2013 accession. The cumulative all-migrant-worker
policy-score coefficient is
...
Eurostat\textquotesingle s annual permit-flow outcomes. The resulting
country-year policy measure is denoted Restrict. It is constructed by
multiplying the signed direction of each event by its magnitude weight,
giving fine-tuning reforms a weight of one and major reforms a weight of
four.
...
the annual policy-flow scores from the 2007 baseline through year t.
Although permit outcomes are observed from 2008 to 2020,
...
\Needspace{7\baselineskip}\captionof{table}{Preferred all-migrant-worker cumulative policy measure by policy tool.}\label{tab:tools}
