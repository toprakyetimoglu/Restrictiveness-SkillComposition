...
origin-year, and Robustness 6 additionally controls for the directive
...
together in Table~\ref{tab:robustness}. Panel A contains checks that
re-estimate or directly modify the preferred denominator-weighted
bilateral model. Panel B reports the two aggregate EU27 placebo
comparisons that remain useful as falsification diagnostics. The panels
answer different questions, so their coefficients should not be read as
estimates from one common model. The overall pattern is mixed but
informative: the preferred bilateral result survives the no-UK
...
while the aggregate placebo evidence is cautionary rather than
confirmatory.
...
framework; Panel B provides limited aggregate falsification diagnostics. Bilateral and aggregate coefficients are not directly
...
The cumulative EU27 placebo does not provide positive falsification
support. Its coefficient is positive and marginal at the
...
The annual-flow EU27 placebo is also positive and marginal at the
ten-percent level (\(\beta =\) 0.010176, p = 0.076), again on only 38
...
Robustness 5 isolates the single most-targeted policy instrument. The
...
Robustness 6 addresses a related concern that European destinations
...
\captionof{figure}{Coefficient estimates across main, robustness, and placebo specifications. Coefficients are shown in percentage-point units; bars are 95 percent confidence intervals from reported standard errors. Preferred bilateral rows report the all-migrant-worker coefficient from specifications that control for skilled/high-skilled and low-skilled target-group policy changes. Robustness rows include the all-first-permits denominator, modeled ILO unemployment with and without the United Kingdom, the core labor-entry measure, the high-income-origins stratum from the origin-income heterogeneity check (Robustness 4), the work-visa-only Restrict narrowing (Robustness 5), and legally applicable EU directive-window controls (Robustness 6). The placebo row reports the limited aggregate EU27 cumulative diagnostic.}\label{graph:coefficients}

Graph 2 reinforces the tables: evidence is strongest in the controlled
bilateral specifications, while the aggregate estimates and UK-inclusive
check are not significant. The EU27 placebo remains weak and the
broad-denominator check secondary.
...
One methodological extension follows from these limitations. The
